...
sed -i '' 's/EMAIL_FROM=smartcommunity/EMAIL_FROM=1217112842@qq.com/' backend/.env.development
...
curl -X POST http://localhost:3001/api/auth/send-verification-code \
  ...
  -d '{"identifier":"1217112842@qq.com","type":"email"}'
```
...
**解决方法**：将 `.env.development` 文件中的 `EMAIL_FROM` 修改为有效的邮箱地址格式 `1217112842@qq.com`。
